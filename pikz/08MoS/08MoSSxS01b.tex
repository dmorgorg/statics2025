...
\begin{tikzpicture}[scale=\scale, line cap=round]
	...
	\only<3->{
		...
		\draw[very thick, -latex] (C)--+(180:1.4)node[left,xshift=0.5mm]{$F_{BC}$};		
		\draw[very thick, -latex] (E)--+(180:1.0625)node[left,xshift=0.5mm]{$F_{EF}$};		
		\draw[very thick, -latex] ($(E)+(135:0)$)--+(135:1.5)node[xshift=-2mm, yshift=2.5mm]{$F_{BE}$};
	}	
	\only<3>{
		\tikzset{outer sep=-0.25mm}
	...
	% stop the image shift which occurs if these are removed rather than blanked
	\only<4->{
