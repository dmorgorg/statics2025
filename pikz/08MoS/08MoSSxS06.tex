...
\begin{tikzpicture}[scale=\scale, line cap = round]
	...
	\coordinate (BC) at ($(B)!0.6!(C)$);
	\coordinate (CH) at ($(H)!0.5!(C)$);
	\coordinate (GH) at ($(H)!0.4!(G)$);
	...
	\only<4->{
		\Meme{GH}{H}{Cornsilk3}{white}{gray!50}{0.4}{0.2}{0.125}	
		\Meme{GH}{G}{LightBlue4}{LightBlue3}{black}{0.4}{0.2}{0.125}	
		\Meme{A}{B}{Cornsilk3}{white}{gray!50}{0.4}{0.2}{0.125}
		\Meme{BC}{B}{Cornsilk3}{white}{gray!50}{0.4}{0.2}{0.125}
		\Meme{BC}{C}{LightBlue4}{LightBlue3}{black}{0.4}{0.2}{0.125}
		\Meme{CH}{H}{Cornsilk3}{white}{gray!50}{0.4}{0.2}{0.125}
		\Meme{CH}{C}{LightBlue4}{LightBlue3}{black}{0.4}{0.2}{0.125}
		\Meme{B}{I}{Cornsilk3}{white}{gray!50}{0.4}{0.2}{0.125}
		
		\Meme{I}{H}{Cornsilk3}{white}{gray!50}{0.4}{0.2}{0.125}
		\Meme{B}{H}{Cornsilk3}{white}{gray!50}{0.4}{0.2}{0.125}
	}
	...
	\only<3->{
		\begin{scope}[rotate around = {-7.5:(\bx/2+\cx/2, \by/2+\gy/4+\hy/4)}]
			\fill[white] (\gx+0.75cm, \gy+0.5cm) rectangle (\bx-1.125cm, \by+0.325cm);
		\end{scope}	
	}
	...
	\filldraw[ball color = LightBlue4] (F) circle (3pt)node[xshift=-0.125cm, yshift=0.25cm] {$ F $};
	\draw[thin, gray] ($(F)+(0,0.5)$)--+(0,1.5);
	\filldraw[ball color = LightBlue4] (G) circle (3pt)node[xshift=-0.125cm, yshift=0.25cm] {$ G $};
	\draw[thin, gray] ($(G)+(0,0.5)$)--+(0,1.5);
	\filldraw[ball color = LightBlue4] (H) circle (3pt)node[xshift=0.125cm, yshift=0.25cm] {$ H $};
	\draw[thin, gray] ($(H)+(0,0.5)$)--+(0,1.5);
	\filldraw[ball color = LightBlue4] (I) circle (3pt)node[xshift=-0.125cm, yshift=0.25cm] {$ I $};
	
	\only<4->{
		\foreach \coord in {A,B,H,I}{
			\fill[ball color = Cornsilk2] (\coord) circle (2.5pt);		
		};
		
		\draw[very thick, -latex] ($(C)+(53.1:0.325)$)--+(53.1:2)node[above]{$ F_{CH} $};
		\draw[very thick, -latex] ($(C)+(-33.7:0.325)$)--+(-33.7:1.5)node[below,xshift=-0.3mm]{$ F_{BC} $};
		\draw[very thick, -latex] ($(G)+(0:0.325)$)--+(0:1.5)node[above,xshift=-0.3mm]{$ F_{GH} $};		
	}
	\only<5->{
		\draw[thin, gray] ($(C)+(0:0.7)$)--+(0:1.25);
		\node at ($(C)+(-15:1.25)$){\small $ \theta $};
		\node at ($(C)+(25:1.25)$){\small $ \phi $};
	}
	...
	\only<2>{
		\begin{scope}[rotate around = {-7.5:(\bx/2+\cx/2, \by/2+\gy/4+\hy/4)}]
			\draw[ultra thick,Green3] (\bx/2+\cx/2, \by+0.325cm)--(\hx/2+\gx/2, \gy+0.75cm);
	...
	\foreach \coord in {A,B,C}{
		\filldraw[ball color = LightBlue4] (\coord) circle (3pt)node[below left] {$ \coord $};
	};
	\foreach \coord in {D,E}{
		\filldraw[ball color = LightBlue4] (\coord) circle (3pt)node[left, yshift=-0.25cm] {$ \coord $};
	};

	% \only<8->{
	% 	\filldraw[ball color = LightBlue4] (J) circle (3pt)node[left, yshift=0.25cm] {$\bm J $};
	% }
