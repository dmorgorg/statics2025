...
\newcommand{\Skywalker}[8][1]{
	...
	\tikz[transform canvas={xscale=\xscale}]{
		...
		\Foot[-30]{rt}
		\Foot[-95]{lt}
		\draw[line width= \lwidth mm, \bg, rotate around={-8.25:(ra)}, line cap=round, rounded corners] ($ (ra)+(\lwidth mm, 0)+(0.125*\scale, 0.5*\scale) $) -- ++(0,0.565*\scale) -- +(137:0.35*\scale);
		\draw[line width= \lwidth mm, \bg, rotate around={-7.25:(ra)}, line cap=round, rounded corners] ($ (ra)+(-\lwidth mm, 0)+(-0.13*\scale, 0.5*\scale) $) -- ++(0,0.445*\scale) -- +(144.5:0.4*\scale);
	}
